...
func TestResultBackwardCompatibility(t *testing.T) {
	...
	var result pond.Result[string] = pool.Submit(func() string { return responseSubmit })
	var resultErr pond.Result[string] = pool.SubmitErr(func() (string, error) { return responseSubmitErr, nil })

	resultTry, ok := pool.TrySubmit(func() string { return responseTrySubmit })
	if !ok {
		t.Error("TrySubmit() = false, want true")
	}
	var resultTryTyped pond.Result[string] = resultTry

	resultTryErr, ok := pool.TrySubmitErr(func() (string, error) { return responseTrySubmitErr, nil })
	if !ok {
		t.Error("TrySubmitErr() = false, want true")
	}
	var resultTryErrTyped pond.Result[string] = resultTryErr

	output, err := result.Wait()
	if err != nil || output != responseSubmit {
		t.Errorf("result.Wait() = (%q, %v), want (%q, nil)", output, err, responseSubmit)
	}

	output, err = resultErr.Wait()
	if err != nil || output != responseSubmitErr {
		t.Errorf("resultErr.Wait() = (%q, %v), want (%q, nil)", output, err, responseSubmitErr)
	}

	output, err = resultTryTyped.Wait()
	if err != nil || output != responseTrySubmit {
		t.Errorf("resultTryTyped.Wait() = (%q, %v), want (%q, nil)", output, err, responseTrySubmit)
	}

	output, err = resultTryErrTyped.Wait()
	if err != nil || output != responseTrySubmitErr {
		t.Errorf("resultTryErrTyped.Wait() = (%q, %v), want (%q, nil)", output, err, responseTrySubmitErr)
	}
}
...
func TestResultInterfaceCompatibility(t *testing.T) {
	...
	processResult := func(r pond.Result[int]) (int, error) {
		return r.Wait()
	}

	task := pool.Submit(func() int { return 42 })

	result, err := processResult(task)
	if err != nil {
		t.Errorf("processResult() err = %v, want nil", err)
	}
	if result != 42 {
		t.Errorf("processResult() = %d, want 42", result)
	}
}
